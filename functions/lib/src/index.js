...
exports.onMergeRequestResolved = exports.setUserRole = exports.onUserCreate = void 0;
...
exports.setUserRole = functions.https.onCall(async (request) => {
    ...
    if (callerRole !== "admin" && callerRole !== "superadmin") {
        throw new functions.https.HttpsError("permission-denied", "Only Admins and Super Admins can assign roles.");
    }
    const { targetUid, newRole } = request.data;
    if (!targetUid || !newRole) {
        throw new functions.https.HttpsError("invalid-argument", "Must provide targetUid and newRole.");
    }
    const allowedRoles = ["student", "cr", "admin"];
    if (!allowedRoles.includes(newRole)) {
        throw new functions.https.HttpsError("invalid-argument", `Invalid role: ${newRole}. Allowed roles: ${allowedRoles.join(", ")}`);
    }
    ...
    if (!targetUserData) {
        throw new functions.https.HttpsError("internal", "Target user profile is empty.");
    }
    // Super Admin check
    if (targetUserData.role === "superadmin" && callerRole !== "superadmin") {
        throw new functions.https.HttpsError("permission-denied", "Only a Super Admin can modify another Super Admin.");
    }
    // Update Firestore
 ...
 */
exports.onMergeRequestResolved = functions.firestore
